...
const actions = {
  create: {
    children: function UIForm(props) {
      ...
      return (
        <Form>
          ...
          <Link href="/panel/customergroups" passHref={true}>
    ...
    onSubmit: async (e, props) => {
      ...
      //Validation
      //POST form values
      const res = await fetch("/api/customergroups", {
        method: "POST",
        headers: {
      ...
      if (res.status < 300) {
        ...
        setTimeout(() => {
          router.push("/panel/customergroups");
...
actions.update.onSubmit = async (e, props) => {
  ...
  body.name = name;
  //POST form values
  const res = await fetch(`/api/customergroups/${props.data.id}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function getServerSideProps({ params }) {
  ...
  const baseurl =
    ...
    let r = await fetch(baseurl + "/api/customergroups/" + id, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
